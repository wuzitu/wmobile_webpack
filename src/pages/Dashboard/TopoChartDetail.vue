<template>
    <div>
        ...
        <div class="echartContiner">
            <v-chart class="topo-chart" ref="TopoChart" :option="option" :autoresize="true" :style="echartsStyle" draggable="true" @dragstart="dragEvents.dragstart" @drag="dragEvents.drag" @dragend="dragEvents.dragend" />
...
const dragEvents = {
    dragstart: (e) => {
        ...
        e.dataTransfer.setDragImage(canvas, 0, 0) //一定要在dragstart事件中使用！！！！
        /* 记录鼠标和画布的初始位置 */
        mouseXY = { X: e.pageX, Y: e.pageY } //计算鼠标落下时的初始位置
        MyChartDomXY = { X: MyChartDom.style.left.split("px")[0] * 1, Y: MyChartDom.style.top.split("px")[0] * 1 }
        console.log("mouse", e.pageX, e.pageY)
        console.log("mouse", MyChartDom.style.left, e.page, MyChartDom.style.top)
    },
    drag: (e) => {
        ...
        MyChartDom.style.left = MyChartDomXY.X + e.pageX - mouseXY.X + "px"
        MyChartDom.style.top = MyChartDomXY.Y + e.pageY - mouseXY.Y + "px"
        console.log("mouse", e.pageX, e.pageY)
